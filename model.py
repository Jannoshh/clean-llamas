...
class Unembed(nn.Module):
    ...
    def forward(
            self,
            normalized_resid_final: Float[Tensor, "batch position d_model"]
    ) -> Float[Tensor, "batch position d_vocab"]:
        return einsum(normalized_resid_final, self.W_U, "batch posn d_model, d_model d_vocab -> batch posn d_vocab") + self.b_U
...
class GroupedMultiQueryAttention(nn.Module):
    ...
    def __init__(self, d_model, n_q_heads, n_kv_heads, d_head, init_range, device):
        ...
        self.W_O = nn.Parameter(torch.empty((n_q_heads, d_head, d_model)))
    ...
    def forward(
        ...
        q = einsum(normalized_resid_pre, self.W_Q,
                   "batch posn d_model, n_q_heads d_model d_head -> batch posn n_q_heads d_head")
        k = einsum(normalized_resid_pre, self.W_K,
                   "batch posn d_model, n_kv_heads d_model d_head -> batch posn n_kv_heads d_head")
        v = einsum(normalized_resid_pre, self.W_V,
                   "batch posn d_model, n_kv_heads d_model d_head -> batch posn n_kv_heads d_head")

        # Calculate attention scores, then scale and mask, and apply softmax to get probabilities
        grouped_q = einops.rearrange(
            q, "batch posn n_q_heads d_head -> batch posn (group_size, n_kv_heads) d_head", n_kv_heads=self.n_kv_heads
        )
        attn_scores = einsum(grouped_q, k,
                             "batch posn_q g_size n_kv_heads d_head, batch posn_k n_kv_heads d_head -> batch g_size n_kv_heads posn_q posn_k")
        attn_scores_masked = self.apply_causal_mask(attn_scores / math.sqrt(self.d_head))
        attn_pattern = attn_scores_masked.softmax(-1)

        # Take weighted sum of value vectors, according to attention probabilities
        z = einsum(v, attn_pattern,
                   "batch posn_k n_kv_heads d_head, batch g_size n_kv_heads posn_q posn_k -> batch posn_q g_size n_kv_heads n_head")

        # Calculate output (by applying matrix W_O and summing over heads, then adding bias b_O)
        z = einops.rearrange(z, "batch posn_q g_size n_kv_heads d_head -> batch posn_q (g_size n_kv_heads) d_head")
        attn_out = einsum(z, self.W_O, "batch posn_q n_q_heads d_head, n_q_heads d_head d_model -> batch posn_q d_model")
        return attn_out
...
class TransformerBlock(nn.Module):
    def __init__(self, d_model, n_heads, d_head, d_mlp, norm_eps, init_range, device):
        super().__init__()
        self.norm1 = RMSNorm(d_model, norm_eps)
        self.attention = GroupedMultiQueryAttention(d_model, n_heads, d_head, init_range, device)
...
class LLamaTransformer(nn.Module):
    def __init__(self, d_vocab, d_model, n_heads, d_head, d_mlp, n_layers, norm_eps, init_range, device):
        super().__init__()
        self.embed = Embed(d_vocab, d_model)
        self.blocks = nn.ModuleList([
            TransformerBlock(d_model, n_heads, d_head, d_mlp, norm_eps, init_range, device)
